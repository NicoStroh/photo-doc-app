...
interface Props {
  ...
  floorName: string;
  onBack: () => void;
  onSelectApartment: (apartmentId: number, apartmentName: string) => void;
}
...
export default function SelectApartmentScreen({ floorId, floorName, onBack, onSelectApartment }: Props) {
  ...
  useEffect(() => {
    const fetchApartments = async () => {
      try {
        const response = await axios.get(`http://10.0.2.2:5000/api/location/floors/${floorId}/apartments`);
